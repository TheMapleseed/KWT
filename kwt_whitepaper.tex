...
and provide a reference implementation in Rust.
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{Check JTI bloom filter} before performing any cryptography
        (prevents decryption-cost denial-of-service from replay flood).
  ...
  \item Validate $\mathtt{expires\_at} > \mathtt{now}$.
  \item Validate \texttt{audience} matches server's registered audience.
  \item Add JTI to bloom filter; return validated claims.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{If v1 is required} (e.g., for interoperability with a Rust
        ...
        ($\sim$163\,kB gzipped) or compile the reference Rust implementation
...
\subsection{Recommended Rust Dependencies}

\begin{table}[H]
\centering
\caption{Rust crates for KWT reference implementation}
\small
\begin{tabularx}{\linewidth}{L{3.2cm} L{3.8cm} X}
\toprule
\rowcolor{kwttablehead}
\color{white}\textbf{Crate} &
\color{white}\textbf{Purpose} &
\color{white}\textbf{Notes} \\
\midrule
\texttt{chacha20poly1305} & XChaCha20-Poly1305 (v1/v3) & RustCrypto; constant-time \\
\texttt{aes-gcm}          & AES-256-GCM (v2)           & Uses AES-NI where available \\
\texttt{hkdf} + \texttt{sha2} & HKDF-SHA256            & RFC 5869 compliant \\
\texttt{uuid}             & UUID v7 for JTI            & Time-ordered for bloom filter \\
\texttt{rand}             & CSPRNG nonce generation    & OS entropy source \\
\texttt{zeroize}          & Key material erasure       & Prevents key in swap/core dump \\
\bottomrule
\end{tabularx}
\end{table}
...
The primary limitation of KWT at this stage is the absence of battle-tested
implementations and independent security audits. The recommendation is to treat
this specification as a design target: implement the reference Rust library,
subject it to fuzz testing and professional audit, and deploy behind a feature
